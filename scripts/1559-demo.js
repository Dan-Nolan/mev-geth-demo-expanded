...
const victimWallet = new ethers.Wallet(privateKey);
...
const main = async() => {
    ...
    console.log("Bundle target block no: ", blockNumber + 3)
    const victimNonce = await client.eth.getTransactionCount(victimWallet.address);
    const nonce = await client.eth.getTransactionCount(testWallet.address);
    const txs = [
        // victim deposits into the contract
        await signEIP1559Tx({
            to: contract.address,
            value: RECEIVER_VALUE, // .321 ETH
            fromAddress: victimWallet.address,
            data: "0x", // direct send
            gasLimit: 200000,
            priorityFee: 0,
            privateKey: victimWallet.privateKey.substring(2), // remove 0x in pk
            nonce: victimNonce
        }, client),
        // MEV searcher withdraw
        await signEIP1559Tx({
            to: contract.address,
            value: 0,
            fromAddress: testWallet.address,
            data: contract.interface.encodeFunctionData("withdraw", [DUMMY_RECEIVER]),
            gasLimit: 200000,
            priorityFee: 0,
